...
module _ {A B : Set c} {E : Set a → Set b} (r : A → B → Set) where
  mutual
    data euttF (sim : ITree E A → ITree E B → Set) : ITree E A → ITree E B → Set (lsuc(a ⊔  b ⊔ c)) where
    ...
    record ∞euttF (sim : ITree E A → ITree E B → Set) (t1 : ITree E A) (t2 : ITree E B) : Set (lsuc(a ⊔  b ⊔ c)) where
      inductive
      field
