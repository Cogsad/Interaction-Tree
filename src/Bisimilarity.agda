...
module _ {A B : Set c} {E : Set a → Set b} (r : A → B → Set) where
  mutual
    data euttF {i : Size} (sim : ITree ∞ E A → ITree ∞ E B → Set ) : ITree ∞ E A → ITree ∞ E B → Set (lsuc(a ⊔  b ⊔ c)) where
    ...
    record ∞euttF {i : Size} (sim : ITree ∞ E A → ITree ∞ E B → Set) (t1 : ITree ∞ E A) (t2 : ITree ∞ E B) : Set (lsuc(a ⊔  b ⊔ c)) where
      inductive
      field
        euttForce : ∀{t1 t2} → {j : Size< i} → euttF sim t1 t2
